defmodule MaintainableMapValidation do
  @doc """
  ...
  """
  @spec add_error(MaintainableMapValidation.Dataset.t(), atom, String.t(), Keyword.t()) ::
          MaintainableMapValidation.Dataset.t()
  def add_error(
        %MaintainableMapValidation.Dataset{errors: errors} = dataset,
        field,
        message,
        keys \\ []
      ) do
  ...
  * `:message` - the message on failure, defaults to "must be accepted"
  """
  @spec validate_acceptance(
          MaintainableMapValidation.Dataset.t() | map | struct,
          atom,
          Keyword.t()
        ) :: MaintainableMapValidation.Dataset.t()
  def validate_acceptance(data, field, opts \\ [])

  def validate_acceptance(%MaintainableMapValidation.Dataset{data: data} = dataset, field, opts) do
  ...
  def validate_acceptance(data, field, opts) when is_map(data),
    do: validate_acceptance(MaintainableMapValidation.Dataset.new(data), field, opts)
  ...
  * `:required` - sets whether existence of a confirmation field is
  ...
  def validate_confirmation(%MaintainableMapValidation.Dataset{data: data} = dataset, field, opts) do
  ...
  def validate_confirmation(data, field, opts) when is_map(data),
    do: validate_confirmation(MaintainableMapValidation.Dataset.new(data), field, opts)
  ...
  * `:message` - the message on failure, defaults to "is reserved"
  ...
  def validate_exclusion(
        %MaintainableMapValidation.Dataset{data: data} = dataset,
        field,
        enum,
        opts
      ) do
  ...
  def validate_exclusion(data, field, enum, opts) when is_map(data),
    do: validate_exclusion(MaintainableMapValidation.Dataset.new(data), field, enum, opts)
  ...
  * `:message` - the message on failure, defaults to "has invalid format"
  ...
  def validate_format(data, field, format, opts) when is_map(data),
    do: validate_format(MaintainableMapValidation.Dataset.new(data), field, format, opts)
  ...
  * `:message` - the message on failure, defaults to "is invalid"
  ...
  def validate_inclusion(data, field, enum, opts) when is_map(data),
    do: validate_inclusion(MaintainableMapValidation.Dataset.new(data), field, enum, opts)
  ...
  * `:message` - the message on failure, depending on the value type, is one of
  ...
  """
  @spec validate_length(MaintainableMapValidation.Dataset.t() | map | struct, atom, Keyword.t()) ::
          MaintainableMapValidation.Dataset.t()
  def validate_length(%MaintainableMapValidation.Dataset{data: data} = dataset, field, opts) do
  ...
  def validate_length(data, field, opts) when is_map(data),
    do: validate_length(MaintainableMapValidation.Dataset.new(data), field, opts)
  ...
  * `:message` - the message on failure, defaults to "can't be blank"
  ...
  def validate_required(%MaintainableMapValidation.Dataset{} = dataset, fields, opts) do
  ...
  def validate_required(data, fields, opts) when is_map(data),
    do: validate_required(MaintainableMapValidation.Dataset.new(data), fields, opts)

  defp validate_field_is_required(
         %MaintainableMapValidation.Dataset{data: data} = dataset,
         [field | t],
         message
       ) do
    ...
    dataset =
      if is_nil(value) || is_empty?(value) do
  ...
  defp is_empty?(value) when is_map(value) do
    length(Map.keys(value)) == 0
  end

  defp is_empty?(value) when is_list(value) do
    length(value) == 0
  end

  defp is_empty?(value) when is_binary(value) do
    String.trim(value) == ""
  end

  defp is_empty?(_value), do: false
