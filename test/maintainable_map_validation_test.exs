defmodule MaintainableMapValidationTest do
  use ExUnit.Case, async: true

  import MaintainableMapValidation

  alias MaintainableMapValidation.{Dataset}
  ...
  describe "validate_confirmation/3" do
    ...
    test "validate when value and confirmation value on alternative field match" do
      dataset =
        dataset(%{foo: "bar", foo_cnfrm: "bar"})
        |> validate_confirmation(:foo, confirmation_field: :foo_cnfrm)
  ...
  describe "validate_length/3 with string" do
    test "add length error when value length must match required length" do
      ...
      assert dataset.errors == [
               {:foo,
                {"should be %{count} character(s)", count: 1, kind: :is, validation: :length}}
             ]
      ...
      assert dataset.errors == [
               {:foo,
                {"should be at most %{count} character(s)",
                 count: 1, kind: :max, validation: :length}}
             ]
      ...
      assert dataset.errors == [
               {:foo,
                {"should be at least %{count} character(s)",
                 count: 4, kind: :min, validation: :length}}
             ]
    ...
    test "add length error when value is an empty list" do
      ...
      assert dataset.errors == [
               {:foo, {"should have %{count} item(s)", count: 1, kind: :is, validation: :length}}
             ]
  ...
  describe "validate_length/3 with list" do
    test "add length error when value length must match required length" do
      ...
      assert dataset.errors == [
               {:foo, {"should have %{count} item(s)", count: 2, kind: :is, validation: :length}}
             ]
      ...
      assert dataset.errors == [
               {:foo,
                {"should have at least %{count} item(s)",
                 count: 2, kind: :min, validation: :length}}
             ]
  ...
  describe "validate_required/3 with list" do
    test "validate when value contains a list with items" do
      dataset = dataset(%{foos: ["foo"]}) |> validate_required(:foos)
      assert dataset.errors == []
      assert dataset.valid?
    end

    test "validate when value contains an empty list" do
      dataset = dataset(%{foos: []}) |> validate_required(:foos)
      assert dataset.errors == [{:foos, {"can't be blank", validation: :required}}]
      refute dataset.valid?
    end
  end

  describe "validate_required/3 with map" do
    test "validate when value contains a map with items" do
      dataset = dataset(%{foos: %{foo: "foo"}}) |> validate_required(:foos)
      assert dataset.errors == []
      assert dataset.valid?
    end

    test "validate when value contains an empty map" do
      dataset = dataset(%{foos: %{}}) |> validate_required(:foos)
      assert dataset.errors == [{:foos, {"can't be blank", validation: :required}}]
      refute dataset.valid?
    end
